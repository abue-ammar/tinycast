...
struct ExtensionTests {
    ...
    static func manifestChecks() {
        let json: [String: Any] = [
            ...
            "preferences": [
                ...
                ["name": "socket", "type": "textfield", "default": ["macOS": "/var/run/x.sock", "Windows": "\\\\pipe"]],
        ...
        check("non-extension entry id is rejected", ExtensionCommandRef(entryID: "/Applications/Mail.app") == nil)
    }
    ...
    static func screenChecks() {
        ...
        check("items flattened in order", list.items.map { $0.string("title") } == ["Apple", "Banana", "Cherry"])
    ...
    static func runtimeChecks() async {
        ...
        check(
            "crypto shim",
            ExtensionAccessoriesView_labelForTest(screen.items.first?.array("accessories").first) == "ba7816bf",
    ...
    static func runInstalledExtension(directory: URL, command commandName: String?) async {
        ...
        let settleMS = UInt64(ProcessInfo.processInfo.environment["EXT_TEST_SETTLE_MS"].flatMap(UInt64.init) ?? 1500)
        ...
        print("\(recorder.trees.count) render(s); host calls: \(Set(host.calls).sorted().joined(separator: ", "))")
