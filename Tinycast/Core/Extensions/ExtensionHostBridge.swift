...
struct ExtensionToast: Sendable, Equatable, Identifiable {
    ...
    init(id: Int = 0, style: Style = .success, title: String = "", message: String? = nil, primaryAction: Action? = nil, secondaryAction: Action? = nil) {
        self.id = id
        self.style = style
        self.title = title
...
final class ExtensionHostBridge: ExtensionHostAPI {
    ...
    private func applications(forPath path: String?) -> [[String: Any]] {
        ...
        if let path, !path.isEmpty {
            ...
            urls = AppIndex.installedApplicationURLs()
        }
