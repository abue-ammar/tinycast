...
enum IconCache {
    ...
    // One size per file: replacing a row must not accumulate all three layout sizes.
    ...
    private static func rowIcon(forFile path: String, stamp: Int, size: IconSize) -> NSImage {
        ...
        let generation = styleGeneration.withLock { $0 }
        let key = fileKey(path, stamp)
        let (image, cost) = autoreleasepool {
            let baseline = downsampled(NSWorkspace.shared.icon(forFile: path))
            return resized(baseline, to: size)
        }
        styleGeneration.withLock { current in
            if current == generation {
                rowCache.setObject(RowImage(size: size, image: image), forKey: key, cost: cost)
            }
        }
        return image
    }

    // Preserve AppKit's source selection and shadow before reducing the retained bitmap.
    private static func resized(_ baseline: (NSImage, Int), to size: IconSize) -> (NSImage, Int) {
        guard size.pixels < Int(displayPixel * 2) else { return baseline }
        guard
            let rep = NSBitmapImageRep(
        ...
        guard let context = NSGraphicsContext(bitmapImageRep: rep) else { return baseline }
        NSGraphicsContext.saveGraphicsState()
        NSGraphicsContext.current = context
        context.imageInterpolation = .high
        baseline.0.draw(in: NSRect(origin: .zero, size: rep.size))
        NSGraphicsContext.restoreGraphicsState()
