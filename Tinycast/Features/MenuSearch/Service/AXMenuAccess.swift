...
/// Every menu-bar `AXUIElement` read in the feature: one app's bar, walked cold, never opened.
/// No actor state and no AppKit, so the snapshot walk runs off-main.
enum AXMenuAccess {
    ...
    private static func batch(_ element: AXUIElement) -> (
