...
/// Shared payload mapping keeps both container versions behaviorally aligned.
enum RaycastPayloadMapper {
    enum SnippetContainer {
        case builtinPackage
        case current

        var key: String {
            switch self {
            case .builtinPackage: return "builtin_package_snippets"
            case .current: return "snippets"
            }
        }
    }

    static func result(
        from decrypted: Data, snippets: SnippetContainer
    ) throws -> RaycastImport.Result {
        ...
        let snippets = RaycastSnippetImport.parse(
            (json[snippets.key] as? [String: Any])?["snippets"])
