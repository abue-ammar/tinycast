...
- **A hover label is Tinycast's `tooltip`, never `.help()`**: an AppKit tooltip never appears behind the non-activating palette. It hangs above its control by default; a control in the palette header passes `edge: .bottom`, since above it is off the window, and a label may run to several lines — the chat's attachment pill lists every staged name.
