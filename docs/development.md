...
The app target builds and embeds `ClipboardTextHelper` and `ClipboardCaptureHelper` under
`Contents/Helpers`, signing each on copy. Build the app scheme to include them; copying only the
main executable omits OCR and clipboard capture. Their executable names stay fixed even when
release builds override the app's product name for a channel.
